...
import com.dominate.caravan.ui.account.commercialestateads.adapter.MyAdsCommercialEstateAdapter
...
@AndroidEntryPoint
class CommercialEstateAdsFragment : BaseFragment(), TextWatcher {

    var binding: FragmentRealEstateAdsBinding by autoCleared()
    private val viewModel: CommercialEstateAdsViewModel by viewModels()
    var token: String? = ""
    lateinit var myAdsAdapter: MyAdsCommercialEstateAdapter
    ...
    private fun setData() {
        ...
        binding.textView02.text  = "اعلانات العقارية التجارية"
        try {
    ...
    private fun observeEffective() {
        ...
        viewModel.getEffectiveCommercialEstate(token = token!!)
                ...
                if (it.data?.status?.code == 200) {
                    it.data!!.let { it ->
                        if (it.results.isNotEmpty()) {
                            ...
                            binding.animationView.visibility = View.GONE
                            myAdsAdapter = MyAdsCommercialEstateAdapter(it.results) {}
    ...
    private fun observeUnderReview() {
        ...
        binding.animationView.visibility = View.GONE
        binding.tvNoAds.visibility = View.GONE
        viewModel.getPendingCommercialEstate(token = token!!)
    ...
    private fun observePaused() {
        ...
        binding.animationView.visibility = View.GONE
        binding.tvNoAds.visibility = View.GONE
        viewModel.getPausedCommercialEstate(token = token!!)
    ...
    private fun observeRejected() {
        ...
        binding.animationView.visibility = View.GONE
        binding.tvNoAds.visibility = View.GONE
        viewModel.getRejectedCommercialEstate(token = token!!)
